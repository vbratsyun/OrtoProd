document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll(".prosthesis-list__item").forEach((item) => {
    ...
    const priceValueElement = item.querySelector(
      ".prosthesis-list__price-value--radio"
    );
    ...
    const descriptions = item.querySelectorAll(
      ".prosthesis-list__description[data-option]"
    );

    function updateContentFromCheckedRadio() {
      const checkedRadio = componentsList.querySelector(
        ".components-list__control-radio:checked"
      );
      ...
      const currencyPart =
        priceValueElement.querySelector(".prosthesis-list__footnote-sign")
          ?.outerHTML || "";
      ...
      descriptions.forEach((desc) => {
        desc.style.display =
          desc.getAttribute("data-option") === option ? "" : "none";
      });
    }
